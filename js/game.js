...
var Game = React.createClass({
  ...
  Return value: None
  The function reveal the board recursively */ 
  handleClick:function(i,j){
	  ...
	  if(my_board[i][j] == EMPTY){
		  ...
		  // check if won
		  this.checkWin();
		  // there are mines around me, else recursively find in squares around me
		  if(superman_board[i][j] > 0){
			  // update the number of mines around me
			...
			// Top
			 if (isValid(i-1, j, my_board)){
				if(!isUncoverd(getSquare(i-1,j))){
			 ...
			 // Left
			 if (isValid(i, j-1, my_board)){
				if(!isUncoverd(getSquare(i,j-1))){
			 ...
			 // Bottom Left
			 if (isValid(i+1, j-1, my_board)){
				if(!isUncoverd(getSquare(i+1, j-1))){
